        ...
        case D3D_ROOT_SIGNATURE_VERSION_1_1:
        {
            ...
            if (pParameters)
            {
                for (UINT n = 0; n < desc_1_1.NumParameters; n++)
                {
                    ...
                    {
                        LPVOID range = reinterpret_cast<void*>(const_cast<D3D12_DESCRIPTOR_RANGE*>(pParameters_1_0[n].DescriptorTable.pDescriptorRanges));
                        HeapFree(GetProcessHeap(), 0, range);
                    }
                }
